...
class InvoiceController extends ControllerBase {
  ...
  public function purchaseHistory() {
    ...
    $query->fields('s', ['id','uid','mail','platform','details']);
  ...
  public function createInvoice($id){
    ...
    $query = \Drupal::database()->select('ppss_sales', 's')->condition('id', $id)->condition('uid', $this->currentUser()->id())->fields('s');
    ...
    if($num_rows > 0) {
      ...
      if($invoice > 0) {
        $no_certificado =  $invoice['no_certificado'];
        $uuid =  $invoice['uuid'];
        $pdf =  $invoice['pdf'];
        $xml =  $invoice['xml'];
        ...
        try {
          ...
          if($user > 0) {
            $ppss_sales = \Drupal::database()->select('ppss_sales', 's')->condition('id', $id)->fields('s');
            $result_ppss_sales = $ppss_sales->execute()->fetchAssoc();
            $sale = json_decode($result_ppss_sales['details']);
            # datos basicos SAT
